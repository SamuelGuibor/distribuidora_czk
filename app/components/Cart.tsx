  ...
  import { getOrderItemByProduct } from "../_actions/delete-orderItem"
  ...
  const handleRemove = async (productId: string) => {
    try {
      const orderItem = await getOrderItemByProduct(productId);

      if (orderItem) {
        await getOrderItemByProduct(orderItem.id); // Remove do banco de dados
      } else {
        console.warn("Item não encontrado no orderItem, removendo apenas do carrinho.");
      }

      removeFromCart(productId); // Remove do estado do carrinho
    } catch (error) {
      ...
      try {
        ...
        for (const item of cartItems) {
          ...
          await addOrderItem({
            ...
            price: parseFloat(priceFormatted.replace("R$", "").replace(",", ".")), // Converte para número
          });
        }
